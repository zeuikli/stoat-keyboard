...
>
> **目前維護**：`line-0.1.215`（iOS26，`v0.1.215`）＋ `line-ios18-0.1.215`（iOS18，`v0.1.215-ios18`），詞庫皆 plustrim。**已封存**：`archive/line-0.1.120-frozen`（§133 合併控制列版面）、`archive/main-v0.1.120`（舊基線）；舊穩定 tag `v0.1.141`/`v0.1.141-ios18`、`v0.1.183`（階段七穩定版）保留為歷史快照。
>
...
│  └─ Scripts/           #   select-variant.sh（選詞庫變體）、package-ipa.sh（→ IPA）
├─ RimeData/             # 打包用 active 詞庫（不入庫，由 select-variant.sh 生成）
│  ├─ shared/            #   schema / 詞庫源（bopomo_onion.schema.yaml…）
│  └─ build/             #   預編譯 .bin（prism / table / reverse）
├─ RimeData-variants/    # 三詞庫變體（入庫＝真相源）：{shared,build}-{lite,plustrim,full}
├─ ios-build/            # librime.xcframework 建置（含 selftest）
└─ releases/             # 已封裝 IPA（gitignored，散佈走 GitHub Releases）
```
...
本鍵盤提供**三種詞庫版本，分別打包成獨立 IPA**（不在 App 內即時切換——librime 全域重載會出字異常）。`RimeData/build/` 不入庫，打包前用 `select-variant.sh` 從 `RimeData-variants/` 的 `build-full`（Plus）/ `build-plustrim`（Plus 精選）/ `build-lite`（純注音）生成：
