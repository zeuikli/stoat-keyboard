...
主線 **`line-0.1.119`**（release `v0.1.141`），§130 候選列版面（⚙⌨ 上列、候選+⌄ 下列）。另提供 **iOS 18 風格變體**：

| 風格 | git 分支 / tag | 鍵盤外觀 |
|------|---------------|---------|
| **iOS 26**（預設） | `line-0.1.119` / `v0.1.141` | Liquid Glass 半透材質、squircle 圓角、`systemGray` 語意色 |
| **iOS 18 扁平** | `line-ios18` / `v0.1.141-ios18` | 實心底（`systemGray4/5`）、5pt circular 方鍵、外框圓角**跟隨系統**、配色全 SDK 語意、保留玻璃切換 |

> 兩風格**僅差鍵盤外觀**（由 `KeyboardViewController.flatStyleIOS18` flag 控制）；輸入核心、emoji、詞庫、§130 版面皆共用。`line-0.1.120`（§133 合併控制列版面）已**凍結不再維護**。
>
...
各風格再分 **full**（純注音 + 81 萬 phrases.chtp 擴充詞庫）/ **lite**（純注音核心）兩詞庫，見下方建置。
