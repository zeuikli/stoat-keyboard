...
- **原廠精確配色** — 底色/鍵色照 iOS 26 系統鍵盤**像素量測值**（淺 `#E2E4E8`、深 `#171717`、鍵白/`#3D3D3D`）；上緣圓角貼齊系統外框。
- **深色模式** — `KBColor` 動態色盤，隨系統深淺自動切換（與原廠一致）。
- **版本分層** — iOS 26 套圓角/連續曲線等新視覺；iOS 16–18 走原廠 classic（方正），以「該版原廠長怎樣」分層。
- **Liquid Glass 玻璃按鍵** — iOS 26 可選開關（預設關＝原廠實心鍵；玻璃為實驗性霜白半透）。
...
│  ├─ Keyboard/          #   鍵盤 extension（KeyboardViewController、KBColor 色盤…）
│  ├─ Shared/Rime/       #   RimeBridge.mm（librime C API 的 ObjC++ 薄包裝）
│  ├─ Harness/           #   KBHarness：嵌鍵盤 + 截圖驗證 target（simctl 取色比對原廠）
...
**前置**（`.xcodeproj` 由 `project.yml` 產生、不入庫）
```bash
brew install xcodegen
cd OnionKB && xcodegen generate     # 產生 OnionKB.xcodeproj
```
